...
/*
 * @brief: 基数排序 《算法导论》P100
 *         * 一种直观的想法是按照最高位排序，但这样经过一轮后(即落在10个桶内后），无法再复用技术排序。（相当于弱化了高位数值，而按照最低位来分段了）
 *         * 所以，从低位->高位的顺序，对进行N轮映射(N为最大数的位数)
 * @output: k表示每个元素有k位
 */
...
/*
 * @brief: 桶排序。《算法导论》P102
 *         * 适用于： 序列分布均匀，切分布在[0,1)区间上。
 *         * 核心： 针对规模为N的序列，在[0,1)上划分N个区间，用近似于o(n)的时间完成排序。
 */
void BucketSort(double a[], int len)
{
	double **bucket = new double*[len];
	for (int i = 0; i < len; i++)
	{
		bucket[i] = new double[len/2]; // 假设冲突的最大值为规模的一半
	}

	for (int i = 0; i < len; i++)
	{
		int b_idx = a[i] * len; // input bucket[idx]
		bucket[b_idx][0] = a[i];
	}
